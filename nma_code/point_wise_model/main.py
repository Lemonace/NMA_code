...
def calculate_result(result_generator):
    ...
    for result in result_generator:
        ...
        loss += result['loss'].reshape(-1).tolist()  
    ctr_auc, ctr_auc_jp, ctr_cb, ctr_cb_jp = metrics.roc_auc_score(y_ctr, pred_ctr), metrics.roc_auc_score(y_ctr, ctr), np.sum(pred_ctr) / np.sum(y_ctr),  np.sum(ctr) / np.sum(y_ctr)
    log_loss = np.sum(loss) / len(loss)
    print("log_loss:{}, ctr_auc:{}, ctr_auc_jp:{}, ctr_cb:{}, ctr_cb_jp:{}".format(log_loss, ctr_auc, ctr_auc_jp, ctr_cb, ctr_cb_jp))
