...
class DNN:
    ...
    def _build_model(self, features, labels, mode, params):
        ...
        with tf.name_scope('dnn_model'):
            fc_out = tf.concat([self.input_embedding], axis=2) # Batch_size * POI_NUM * FEAT_NUM
            ...
            fc_out_ctr = tf.concat([fc_out, self.feat_predict], axis=2)
            ...
            ctr_out = tf.squeeze(tf.layers.dense(fc_out_ctr, 1, activation=None, name="final_out_ctr"), axis=2)
            self.ctr = ctr_out
            if not self.train:
